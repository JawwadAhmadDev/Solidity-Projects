// -------------Note-------------
// This file implements a referral mechanism as described in the readme file.
// All the calculations will be done on centeralized system.
// Only data will be stored on decentralized system.
...
contract Referral is Ownable {
    ...
    uint256 private min_investable_amount = 100; // change from 100 -> 100e18
    uint256 private max_investable_amount = 10000; // change from 10000 -> 1000e18
    ...
    function withdrawReward(uint256 _amount) external {
        ...
        require(
            _amount <= rewardOf[caller],
            "Referral-withdrawReward: Entered amount should be less or equal to your reward"
        );

        // transfer tokens in the form of reward to caller.
        token.transfer(caller, _amount);

        // update reward and withdrawn amount
        rewardOf[caller] = rewardOf[caller].sub(_amount);
        totalWithdrawnAmountOf[caller] = totalWithdrawnAmountOf[caller].add(
            _amount
        );
