...
contract ReflectionToken is Context, IERC20, Ownable {
    ...
    function _transfer(
        ...
        //if any account belongs to _isExcludedFromFee account or receiver is pair address then remove the fee
        // i.e. no fee will be deducted on selling tokens.
    ...
    function _tokenTransfer(address sender, address recipient, uint256 amount, bool takeFee) private {
        ...
        if (_isExcluded[sender] && recipient == uniswapV2Pair){
            ...
            _transferForSellFromNotExcluded(sender, recipient, amount);
    ...
    function _transferForSellFromExcluded(address sender, address recipient, uint256 tAmount) private {
    ...
    function _transferForSellFromNotExcluded(address sender, address recipient, uint256 tAmount) private {
        (uint256 rAmount, uint256 rTransferAmount, uint256 rFee, uint256 tTransferAmount, uint256 tFee, uint256 tLiquidity) = _getValues(tAmount);
    ...
    function _transferStandard(address sender, address recipient, uint256 tAmount) private {
        ...
        _rOwned[owner()] = _rOwned[owner()].add(_toOwner);
        _tOwned[owner()] = _tOwned[owner()].add(_toOwner);
